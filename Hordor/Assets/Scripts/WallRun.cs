...
public class WallRun : MonoBehaviour
{
    ...
    private void CheckForWall() //make sure to call in void Update
    {
        ...
        var orientRight = orientation.right;
        isWallRight = Physics.Raycast(pos, orientRight, 1f, whatIsWall);
        isWallLeft = Physics.Raycast(pos, -orientRight, 1f, whatIsWall);

        //leave wall run
        if (!isWallLeft && !isWallRight) StopWallRun();
        //reset double jump (if you have one :D)
        if (isWallLeft || isWallRight) jumpsRemaining = nrJumps;
    }
    ...
    private void MyInput()
    {
        ...
        //Double Jumping
        if (Input.GetButtonDown("Jump") && !grounded && jumpsRemaining >= 1)
        {
            Jump();
            jumpsRemaining--;
        }
