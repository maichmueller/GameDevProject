...
public class Grappler : MonoBehaviour
{
    ...
    private void LaunchHook()
    {
        ...
        // mouse.z = transform.position.z - mainCamera.transform.position.z;
        Vector3 mouseInScene = mainCamera.ScreenToWorldPoint(mouse);

        // Get a ray from the character to the mouse:
        var charPos = character.gameObject.transform.position;
        Ray ray = new Ray(charPos, mouseInScene - charPos);

        // Raycast to see what we hit:
        if (Physics.Raycast(ray, out hookAttachment))
        {
            if (Vector3.Distance(hookAttachment.transform.position, this.gameObject.transform.position) <= hookDistance)
            {
                Debug.Log("HIT: " + hookAttachment.transform.gameObject.name);
    ...
    void Update()
    {
        if (Input.GetMouseButtonDown(0))
        {
            ...
            {
                DetachHook();
                character.sourceCollider.transform.rotation = new Quaternion(0f, 0f, 0f, 0f);
            }
        }
    }
}